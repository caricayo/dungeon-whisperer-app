-- Enhanced security measures for user_settings table

-- 1. Create audit logging trigger for user_settings access
...
-- Create audit triggers for user_settings
CREATE TRIGGER audit_user_settings_insert
    AFTER INSERT ON public.user_settings
    FOR EACH ROW EXECUTE FUNCTION public.audit_user_settings_access();

CREATE TRIGGER audit_user_settings_update
    AFTER UPDATE ON public.user_settings
    FOR EACH ROW EXECUTE FUNCTION public.audit_user_settings_access();

CREATE TRIGGER audit_user_settings_delete
    AFTER DELETE ON public.user_settings
    FOR EACH ROW EXECUTE FUNCTION public.audit_user_settings_access();

-- 2. Create security definer function to safely retrieve user settings without exposing encrypted keys
...
-- 3. Create function to securely update user settings with validation
...
-- 4. Create function to safely delete encrypted API keys
...
-- 5. Add additional RLS policies for enhanced security
CREATE POLICY "user_settings_no_anonymous_access" 
ON public.user_settings 
FOR ALL 
TO anon
USING (false);

-- 6. Create rate limiting for user_settings operations
...
GRANT EXECUTE ON FUNCTION public.get_safe_user_settings(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_user_settings_secure(text, text, text, numeric, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_user_api_key() TO authenticated;
GRANT EXECUTE ON FUNCTION public.check_user_settings_rate_limit() TO authenticated;