...
-- Add soft delete and room columns to sessions
...
BEGIN
  ...
  SELECT s.id, s.name, s.custom_prompt, s.messages, s.max_players, s.is_multiplayer, 
         s.current_player_count, s.room_id, s.status, s.campaign_id
  INTO session_record
  ...
  IF NOT EXISTS (
    ...
    -- Update player count via trigger
  END IF;
  ...
  SELECT jsonb_build_array(
    SELECT jsonb_array_elements(session_record.messages) 
    ORDER BY (jsonb_array_elements(session_record.messages)->>'timestamp')::timestamp DESC 
    LIMIT 30
  ...
  ) INTO room_record FROM public.session_participants sp
  ...
  RETURN jsonb_build_object(
    ...
    'members', COALESCE(room_record, '[]'::jsonb),
    'last30Messages', COALESCE(last_30_messages, '[]'::jsonb),
