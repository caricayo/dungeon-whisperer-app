-- ============================================================================
-- ROLE CONSTRAINT FIX - Run this in Supabase SQL Editor
-- ============================================================================
-- This fixes the "session_participants_role_check" constraint violation
-- by changing 'admin' role to 'dm' role (which is allowed by the constraint)
-- ============================================================================

-- Drop and recreate the create_multiplayer_session function with correct role
DROP FUNCTION IF EXISTS public.create_multiplayer_session(text, text, integer);
...
BEGIN
  ...
  BEGIN
    ...
    -- Add creator as the first participant with dm role (FIXED: was 'admin', now 'dm')
    ...
    VALUES (
      new_session_id,
      current_user_id,
      'dm',         -- FIXED: Use 'dm' role instead of 'admin' to satisfy constraint
      jsonb_build_object(
        'can_invite', true,
        'can_kick', true,
...
-- ============================================================================
-- 
-- ❌ PROBLEM: Function tried to insert role='admin' 
-- ✅ SOLUTION: Changed to role='dm' (allowed by constraint)
--
-- The session_participants table has a CHECK constraint:
-- CHECK (role IN ('dm', 'player'))
--
-- This fix ensures the creator gets 'dm' role with full permissions
-- ============================================================================