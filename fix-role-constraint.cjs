...
const fixedCreateSessionFunction = `
...
BEGIN
  ...
  BEGIN
    ...
    -- Add creator as the first participant with dm role (not admin)
    ...
    VALUES (
      new_session_id,
      current_user_id,
      'dm',         -- Use 'dm' role instead of 'admin'
      jsonb_build_object(
        'can_invite', true,
        'can_kick', true,
...
async function deployFix() {
  console.log('🔧 Deploying role constraint fix...');
  
  try {
    const { data, error } = await supabase.rpc('exec_sql', {
      sql: fixedCreateSessionFunction
    });
    
    if (error) {
      console.error('❌ Failed to deploy fix:', error);
      return;
    }
    
    console.log('✅ Role constraint fix deployed successfully!');
    console.log('📋 Function updated to use "dm" role instead of "admin"');
