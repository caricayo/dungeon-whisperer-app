import { performance_monitor } from '@/lib/performance';
import { env } from '@/lib/env';

/**
 * Image optimization utilities
 */
const QUALITY_SETTINGS = {
  thumbnail: { width: 150, height: 150, quality: 70 },
  medium: { width: 600, height: 400, quality: 80 },
  large: { width: 1200, height: 800, quality: 85 },
};

/**
 * Create responsive image with lazy loading
 */
export function createResponsiveImage(src: string, alt: string, className?: string) {
  return {
    src,
    alt,
    loading: 'lazy' as const,
    decoding: 'async' as const,
    className: `transition-opacity duration-300 ${className ?? ''}`,
    onLoad: (e: React.SyntheticEvent<HTMLImageElement>) => {
      e.currentTarget.style.opacity = '1';
    },
    onError: (e: React.SyntheticEvent<HTMLImageElement>) => {
      e.currentTarget.src = '/placeholder.svg';
      e.currentTarget.alt = 'Image failed to load';
    },
  };
}

/**
 * Preload critical images
 */
export function preloadImages(urls: string[]): void {
  if (env.VITE_APP_ENV === 'development') {
    console.warn('🖼️ Preloading', urls.length, 'critical images');
  }
  
  urls.forEach(url => {
    const img = new Image();
    img.src = url;
  });
}

/**
 * Optimize image based on device capabilities
 */
export function getOptimalImageProps(
  baseUrl: string, 
  alt: string, 
  size: keyof typeof QUALITY_SETTINGS = 'medium'
) {
  if (!(size in QUALITY_SETTINGS)) {
    throw new Error(`Invalid image size: ${size}`);
  }
  const settings = QUALITY_SETTINGS[size];
  const isRetina = window.devicePixelRatio > 1;
  
  return {
    ...createResponsiveImage(baseUrl, alt),
    width: settings.width,
    height: settings.height,
    srcSet: isRetina ? `${baseUrl} 1x, ${baseUrl} 2x` : undefined,
  };
}

export const ImageOptimizer = {
  createResponsiveImage,
  preloadImages,
  getOptimalImageProps
};

/**
 * Code splitting utilities for dynamic imports
 */
const loadingComponents = new Set<string>();

/**
 * Dynamically import component with loading state
 */
export async function loadComponent<T>(
  importFn: () => Promise<{ default: T }>,
  componentName: string
): Promise<T> {
  if (loadingComponents.has(componentName)) {
    // Prevent duplicate loading
    while (loadingComponents.has(componentName)) {
      await new Promise(resolve => setTimeout(resolve, 50));
    }
  }

  loadingComponents.add(componentName);
  
  try {
    const start = performance.now();
    const module = await importFn();
    const duration = performance.now() - start;
    
    if (env.VITE_APP_ENV === 'development') {
      console.warn(`📦 Loaded ${componentName} in ${duration.toFixed(2)}ms`);
    }
    
    return module.default;
  } finally {
    loadingComponents.delete(componentName);
  }
}

/**
 * Preload components for better UX
 */
export function preloadComponent(importFn: () => Promise<{ default: unknown }>, componentName: string): void {
  // Preload on idle
  if ('requestIdleCallback' in window) {
    requestIdleCallback(() => {
      loadComponent(importFn, componentName).catch(console.warn);
    });
  } else {
    setTimeout(() => {
      loadComponent(importFn, componentName).catch(console.warn);
    }, 2000);
  }
}

export const CodeSplitter = {
  loadComponent,
  preloadComponent
};

/**
 * Font loading optimization
 */
let fontsLoaded = false;

/**
 * Fonts are already loaded in index.html - skip duplicate loading
 */
export function preloadFonts(): void {
  if (fontsLoaded || typeof document === 'undefined') return;
  
  // Fonts are already preloaded in index.html, just mark as loaded
  fontsLoaded = true;
  
  if (env.VITE_APP_ENV === 'development') {
    console.warn('✅ Fonts already loaded via index.html');
  }
}

/**
 * Add font display swap for better loading performance
 */
export function addFontDisplaySwap(): void {
  if (typeof document === 'undefined') return;

  const style = document.createElement('style');
  style.textContent = `
    @font-face {
      font-family: 'Cinzel';
      font-display: swap;
    }
    @font-face {
      font-family: 'Inter';
      font-display: swap;
    }
    @font-face {
      font-family: 'Playfair Display';
      font-display: swap;
    }
  `;
  document.head.appendChild(style);
}

export const FontOptimizer = {
  preloadFonts,
  addFontDisplaySwap
};

/**
 * Resource prioritization for critical rendering path
 */

/**
 * Resource hints already set in index.html - skip duplicates
 */
export function addResourceHints(): void {
  if (typeof document === 'undefined') return;
  
  // Resource hints already set in index.html, just add additional ones
  const prefetchDomains = [
    'https://api.openai.com',
  ];

  prefetchDomains.forEach(domain => {
    const link = document.createElement('link');
    link.rel = 'dns-prefetch';
    link.href = domain;
    document.head.appendChild(link);
  });
  
  if (env.VITE_APP_ENV === 'development') {
    console.warn('✅ Resource hints configured');
  }
}

/**
 * Initialize all performance optimizations
 */
export function initializeOptimizations(): void {
  performance_monitor.start('performance_init');
  
  // Font optimization
  preloadFonts();
  addFontDisplaySwap();
  
  // Resource hints
  addResourceHints();
  
  // Critical images preload
  preloadImages([
    '/placeholder.svg',
    // Add other critical images
  ]);

  performance_monitor.end('performance_init');
  
  if (env.VITE_APP_ENV === 'development') {
    console.warn('🚀 Performance optimizations initialized');
  }
}

export const ResourcePrioritizer = {
  addResourceHints,
  initializeOptimizations
};